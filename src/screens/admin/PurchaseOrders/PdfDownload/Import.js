...
const getDocumentsRequired = (documents) => {
    const requiredDocs = [...new Set(documents.filter(document => document.requiredx)?.map(doc => doc?.doc_kind))]
        ...
        <View style={styles.table}>
            ...
            {data.map((row, i) => (
                <View key={i} style={styles.row} wrap={false}>
                    ...
                    <CustomText style={styles.row23}>{row.container}</CustomText>
        ...
        <Document>
            <Page style={styles.section} size={"A4"}>
                ...
                <View style={styles.podetails}>
                    ...
                    <View style={{ flexDirection: 'row', display: 'flex', marginVertical: 3, }}>
                                    ...
                                    {purchaseDetails?.supplier_address
                                        ...
                                        ?.map((item, index) => {
                                            if (item !== "") {
                                                return (
                                                    <>
                                                        {item.includes("GST NO") ? <Text style={[styles.textregular, {
                                                            textTransform: 'uppercase', width: 'auto', lineHeight: '1.3',
                                                            fontFamily: 'Helvetica-Bold'
                                                        }]}>{item + ','}</Text> :
                                                            item.includes("PAN NO") ? <Text style={[styles.textregular, {
                                                                textTransform: 'uppercase', width: 'auto', lineHeight: '1.3',
                                                                fontFamily: 'Helvetica-Bold'
                                                            }]}>{item + ','}</Text> :
                                                                <Text style={{ textTransform: 'capitalize', lineHeight: '1.3', fontSize: '8px' }}> {item + ","} </Text>}
                                                        {(index >= purchaseDetails.supplier_address.split(",").length - 4) && "\n"}
                                                    </>
                                                );
                                            }
                                        })}
                        ...
                        <View style={{ flex: 1, paddingLeft: 80 }}>
                            ...
                            <KeyValue label={"DESTINATION"} value={purchaseDetails.place_of_destination ? purchaseDetails.place_of_destination : '-'} />
                ...
                <View style={styles.description}>
                    <Text style={[styles.textregular, { textTransform: 'uppercase', width: 'auto', lineHeight: '1.3', fontFamily: 'Helvetica-Bold', textDecoration: 'underline' }]}>
                        SHIPPER DETAILS
                    </Text>
                    <Text style={[styles.textregular, { textTransform: 'uppercase', marginTop: '10px', width: 'auto', lineHeight: '1.3', fontWeight: '300' }]}>
                        PT SARIMKMUR TUNGAL MANDARI
                    </Text>
                </View>
                ...
                <View>
                    ...
                    <CoffeeTable data={[{
                        ...
                        container: purchaseDetails.no_of_containers === '' ? parseInt(purchaseDetails.no_of_containers) : 0 + ' X ' + purchaseDetails.container_type === '' ? parseInt(purchaseDetails.container_type) : 0,
