...
function createData(pono, vendorname, vendortype, pocat, status, currencycode, povalue, podate, total_quantity, gc_itemname) {
  return { pono, vendorname, vendortype, pocat, status, currencycode, povalue, podate, total_quantity, gc_itemname };
}
...
const PurchaseOrderList = (props) => {
  ...
  useEffect(() => {
    ...
    props.data && props.data.map(v => {
      rows.push(createData(v.pono, v.vendorname, v.vendortype, v.pocat, v.status, v.currencycode, v.povalue, formatDate(v.podate), v.total_quantity, v.gc_itemname))
    })
    setAllData(rows);
  }, [props.data]);
  // eslint-disable-next-line
