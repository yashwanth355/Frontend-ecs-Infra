...
const EditAccount = (props) => {
    ...
    useEffect(() => {
        getLeadsInfo({ "type":"accountDetails" }).then(res => setAccountType(formatToSelection(res, 'accounttype')));
        getLeadsInfo({ "type":"productsegments" }).then(res => setProduct(formatToSelection(res, 'productsegment')));    
        getLeadsInfo({ "type":"coffeetypes" }).then(res => setCoffeeType(formatToSelection(res, 'coffeetype')));        
   
        getAccountDetails({
            "viewaccount": true,
            "accountid": props.id.toString()
            }).then(viewPayload => {
                getLeadsInfo({ "type":"countries", "continentname": viewPayload.shippingtocontinent }).then(response => {
               ...
               let temp = {
                ...
                accountType: viewPayload.accounttypes !== null ? formatToSelection(viewPayload.accounttypes, 'accounttype') : [],         
                ...
                productSegment: viewPayload.Productsegment !== null ? formatToSelection(viewPayload.Productsegment, 'productsegment') : [],
                street: (viewPayload.shippinginformation && viewPayload.shippinginformation[0]?.shipping_street) || '',
                city: (viewPayload.shippinginformation && viewPayload.shippinginformation[0]?.shipping_city) || '',
                state: (viewPayload.shippinginformation && viewPayload.shippinginformation[0]?.shipping_state) || '',
                postalCode: (viewPayload.shippinginformation && viewPayload.shippinginformation[0]?.shipping_postalcode) || '',
                country: (viewPayload.shippinginformation && viewPayload.shippinginformation[0]?.shipping_country) || '',
                billingStreet: (viewPayload.billinginformation && viewPayload.billinginformation[0]?.billing_street) || '',
                billingCity: (viewPayload.billinginformation && viewPayload.billinginformation[0]?.billing_city) || '',
                billingState: (viewPayload.billinginformation && viewPayload.billinginformation[0]?.billing_state) || '',
                billingPostalCode: (viewPayload.billinginformation && viewPayload.billinginformation[0]?.billing_postalcode) || '',
                billingCountry: (viewPayload.billinginformation && viewPayload.billinginformation[0]?.billing_country) || '',
                alreadyInBusiness: viewPayload.profilesectioninfo && viewPayload.profilesectioninfo[0]?.instcoffee === 1 ? "Yes" : "No" || 'No',
                hasManfacUnit: viewPayload.profilesectioninfo && viewPayload.profilesectioninfo[0]?.manfacunit === 1 ? "Yes" : "No"|| 'No',
                 // eslint-disable-next-line
                coffeeType: viewPayload.coffeetypes !== null ? formatToSelection(viewPayload.coffeetypes, 'coffeetype') : [],
                other: viewPayload.otherinfo || '',
                shippingCountry: {label: viewPayload.shippingtocountry, value: viewPayload.shippingtocountry},
                shippingContinent: {label: viewPayload.shippingtocontinent, value: viewPayload.shippingtocontinent}    
            }; 
    ...
    <Grid container xs={12} md={12} style={{margin:24}} justify="center">
        <Grid item>
            <Button disabled={loading} label={loading ? 'Loading ...' : 'Save'} onClick={editAccountAction}/>
        </Grid>
        <Grid item>
