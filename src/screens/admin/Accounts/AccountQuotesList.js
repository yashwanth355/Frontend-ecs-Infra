...
const AccountQuotesList = (props) => {
  ...
  React.useEffect(() => {
    ...
    props.data && props.data !== null && props.data.map(v => {
      tempRows.push(createData(v.quoteid, v.status, v.createdby, formatDate(v.createddate)))
    })
    setRows(tempRows);
    // eslint-disable-next-line
  }, [props.data]);
                  ...
                  {columns.map((column) => {
                    ...
                    return (
                      <TableCell key={column.id} align={column.align}>
                        {(column.id === 'number') ?
                          // eslint-disable-next-line
                          <a href='#' onClick={(e) => { CellClickHandler(e, column.id, row.number, row.status) }}>{value}</a>
                          : value
                        }
                      </TableCell>
                    )
