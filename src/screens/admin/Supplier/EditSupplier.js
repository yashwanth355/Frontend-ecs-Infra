...
export default function EditSupplier(props) {
  ...
  useEffect(() => {
    let temp = { ...props.supplierData };

    setLogData(props.supplierData.audit_log_vendor)
    ...
    if (props.supplierData.country !== "") {
      getStateNames({ countryname: props.supplierData.country })
        .then((res) => setStateName(formatToSelection(res, "state")))
        .catch((err) => console.log("Get state error", err.message));
    }
    if (props.supplierData.state !== '') {
      getCityNames({ statename: props.supplierData.state })
