...
import "../../common.css";
import { getGcCreationInfo, createOrUpdateGcDetail } from "../../../apis";
...
const EditGc = (props) => {
  ...
  useEffect(() => {
    getGcCreationInfo({
      ...
      var temp = res.filter(v => v.group_id === props.data.group_id);
  ...
  const updateGcAction = async (e) => {
    e.preventDefault();
    console.log("gcDetails::", gcDetails)
  ...
  let component = (
    <form className={classes.root} noValidate autoComplete="off">
      ...
      <Grid container xs={12} md={12} style={{ margin: 24 }} justify="center">
        <Grid item>
          <Button disabled={loading} label={loading ? "Loading..." : "Save"} onClick={updateGcAction} />
        </Grid>
        <Grid item>
