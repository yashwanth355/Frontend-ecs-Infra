...
export default function GcDetails(props) {
  ...
  async function fetchData() {
    let response = await getgcDetails({ item_id: props.id });
  ...
  const updateGcAction = async (e) => {
    e.preventDefault();
    props.updateGcActionInfo(gcDetails);
  };
    ...
    <form className={classes.root} noValidate autoComplete="off">
        ...
        <CardContent>
          <Grid container md={12}>
            <Grid item md={3} xs={12}>
              ...
              <Typography>{props.id ? props.id : "-"}</Typography>
            </Grid>
            <Grid item md={3} xs={12}>
      ...
      <Grid id="top-row" container>
        <Grid item md={12} xs={12}>
          ...
          >
            <Grid item>
              <Button label="Edit" onClick={updateGcAction} />
            </Grid>
            <Grid item>
