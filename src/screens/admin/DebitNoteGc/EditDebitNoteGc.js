...
import { getinsertOrEditGCDebitNoteDetail, getMrinCreationDetail, getGCDebitNoteCreationInfo, getupdateGcDebitNoteStatus, releaseDebitNoteGC } from '../../../apis';
...
const EditDebitNoteGc = (props) => {
    ...
    useEffect(async () => {
        if (props.data.debit_noteid) {
            ...
            temp.vendor = { vendor_id: props.data.vendor_id, vendor_name: props.data.vendor_name };
    ...
    const StatusChange = async () => {
        try {
            ...
            let response = await getupdateGcDebitNoteStatus({ "type": status, "debit_noteid": props.id, "updated_by": localStorage.getItem('currentUserId') });
            if (response) {
                setSnack({
                    open: true,
                    message:
        ...
        <form className={classes.root} noValidate autoComplete="off">
                        ...
                        <Grid id="top-row" xs={2} md={2} container direction="column">
                            <Grid item>
                                <Button
                                    ...
                                    label={`${ isloadingPdf ? "Releasing ..." : "Release Debit Note"
                                        }`}
                                    onClick={exportPDF}
                                />
                            </Grid>
            ...
            {debitNoteGC !== null &&
                ...
                <div id="vendor" style={{ display: "none" }}>
                  <VendorPDF id={props.id} debitNoteGcDetails={debitNoteGC} />
                </div>
                <div id="account" style={{ display: "none" }}>
                  <AccountPDF id={props.id} debitNoteGcDetails={debitNoteGC} />
                </div>
              </>
            }
